...
if [[ -z "${fixture_id}" ]]; then
  ...
  echo "  scripts/match-stream.sh 123456" >&2
  echo "Or set FIXTURE_ID=..." >&2
  exit 2
fi

seconds="${STREAM_SECONDS:-8}"
mode="${STREAM_MODE:-all}" # all|score|commentary

case "${mode}" in
  all) path="/api/match/${fixture_id}/stream" ;;
  score) path="/api/match/${fixture_id}/score-stream" ;;
  commentary) path="/api/match/${fixture_id}/commentary-stream" ;;
  *)
    echo "Unknown STREAM_MODE='${mode}'. Use: all|score|commentary" >&2
    exit 2
    ;;
esac
